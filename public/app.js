class LLMChatViewer {
  ...
  async fetchModelInfo() {
    try {
      ...
      if (data.llm1) {
        this.displayModelInfo(data.llm1, "llm1Details")
      }
      if (data.llm2) {
        this.displayModelInfo(data.llm2, "llm2Details")
      }
    } catch (error) {
      console.error("Failed to fetch model info:", error)
      this.elements.llm1Details.textContent = "Failed to load model info"
      this.elements.llm2Details.textContent = "Failed to load model info"
    }
  }

  displayModelInfo(modelInfo, elementId) {
